...
async function getPoll(shortId: string) {
  ...
  const res = await fetch(`${base}/api/polls/${shortId}`, {
    cache: 'no-store',
  });
    ...
    <>
      ...
      <main style={{ position: 'relative', zIndex: 1, padding: '120px 24px 80px', minHeight: '100vh' }}>
        <div style={{ maxWidth: 680, margin: '0 auto' }}>
          ...
          {justCreated && (
            <div style={{
              background: 'rgba(107,255,228,0.08)',
              border: '1px solid rgba(107,255,228,0.25)',
              ...
              <div>
                <div style={{ fontFamily: 'Syne, sans-serif', fontWeight: 700, color: '#6BFFE4', fontSize: 14, marginBottom: 4 }}>
                  ✓ Poll created! Share the link below.
                </div>
                <div style={{ fontSize: 12, color: '#6B6B8A', fontFamily: 'DM Mono, monospace', wordBreak: 'break-all' }}>
                  {pollUrl}
                </div>
              </div>
          ...
          {/* Creator controls (rendered client-side, checks localStorage) */}
